...
from utils import (
    ...
    get_stack_outputs, exit_with_error, exit_with_success
)
...
def check_config_drift(existing_config: Dict[str, Any], auth_outputs: Dict[str, str], static_outputs: Dict[str, str]) -> bool:
    ...
    if existing_config.get('userPoolClientId') != auth_outputs.get('UserPoolClientId'):
        print_warning(f"Client ID drift: {existing_config.get('userPoolClientId')} → {auth_outputs.get('UserPoolClientId')}")
        drifted = True
